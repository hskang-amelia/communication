...
/// It generate the validator struct and the type-state chain for the producer,
/// ensuring that all required fields and handlers are properly set before offering the service.
/// User need to call `init()` on the producer to start the type-state chain, and then call the generated
/// `update_*` and `register_set_handler_*`, `register_*_handler` methods in any order,
/// and finally call `offer()` to complete the chain.
/// user will get compile-time error if any required field or handler is not set before calling `offer()`.
///
/// User no need to use this macro explicitly,
/// it will be automatically generated by the `interface!` macro for the producer struct.
///
/// Note: This macro identifies member types by the last segment of each field's type path:
...
/// So if member type is changed to a different type or renamed,
